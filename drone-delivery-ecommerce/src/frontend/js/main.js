...
function showDroneAssignmentModal() {
    ...
    setTimeout(() => {
        ...
        // Add backup event listener to confirm button
...
function confirmModalDroneAssignmentWithId(modalId) {
    ...
    if (orderIndex !== -1) {
        ...
        // Show delivery route immediately after assignment
    ...
    orders.forEach(order => {
        if (order.deliveryLocation && order.deliveryLocation.lat && order.deliveryLocation.lng) {
            try {
                ...
                window.orderMarkers[order.id] = L.marker([order.deliveryLocation.lat, order.deliveryLocation.lng], { icon: icon })
                    .addTo(window.adminDeliveryMap);
...
function animateDroneMovement(droneId, order) {
    ...
    if (droneIndex === -1 || !order.deliveryLocation) {
...
function showDeliveryRoute(orderId) {
    ...
    if (order && order.assignedDrone && order.deliveryLocation) {
        const drones = JSON.parse(localStorage.getItem('drones') || '[]');
        ...
        showNotification('Đơn hàng chưa có drone được phân công hoặc chưa có địa chỉ giao hàng!', 'error');
    }
}
